...
public class PlayePotionEffectAddTime extends PlayerPotionEffect
{
    @Override
    public String getIdentifier() {
        return "PlayerPotionEffectAddTime";
    }
	...
	public void execute()
	{
		...
		{		    
			...
			int amp = vals.length>=3 ? nUtil.isNum(vals[2]) ? Integer.valueOf(vals[2]) : 0 : 0;			
			...
			{
			    duration += player.getPotionEffect(type).getDuration();
			    ambient = player.getPotionEffect(type).isAmbient();
			    particles = player.getPotionEffect(type).hasParticles();
